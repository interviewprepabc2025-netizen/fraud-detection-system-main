"""Behavioral Agent — service entrypoint."""
...
from agents.behavioral.agent import evaluate

configure_logging()
logger = get_logger(__name__)

app = FastAPI(title="Behavioral Agent", version="1.0.0")


@app.get("/health")
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "behavioral"}


def _kafka_loop() -> None:
    producer = make_producer()
    consumer = make_consumer(group_id="behavioral-group")
...
def main() -> None:
    ...
    t.start()
    logger.info("behavioral_agent_starting", port=int(os.getenv("AGENT_PORT", "8003")))
