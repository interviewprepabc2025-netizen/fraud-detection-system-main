"""Network Agent — service entrypoint."""
...
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "network"}


def _kafka_loop() -> None:
    producer = make_producer()
    consumer = make_consumer(group_id="network-group")
...
def main() -> None:
    ...
    t.start()
    logger.info("network_agent_starting", port=int(os.getenv("AGENT_PORT", "8004")))
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("AGENT_PORT", "8004")))
